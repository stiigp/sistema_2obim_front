...
import { consultarUsuarios, deletarUsuario, zerarMensagem } from "../../../redux/redux.usuario";
...
export default function TabelaUsuarios(props) {
    ...
    function atualizarUsuario(user) {
        if (
            window.confirm(
                "Deseja realmente alterar o usuário -> " + user.nome,
            )
        ) {
            props.setUsuarioSelecionado(user);
    ...
    return (
        <>
            <Container>
                {usuario.logado && usuario.perfil === "Admin" && <Button
                    className="mb-3"
                    variant="primary"
                    onClick={() => {
                        props.setExibirTabela(false);
                        ...
                        {listaUsuarios?.map((user) => {
                            return (
                                <tr key={user.codigo}>
                                    ...
                                    <td>{user.nome}</td>
                                    <td>{user.perfil}</td>
                                    {usuario.logado && usuario.perfil === "Admin" && <td>
                                        <Button
                                            onClick={() => {
